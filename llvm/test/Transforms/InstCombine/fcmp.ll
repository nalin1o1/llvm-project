...
define i1 @fcmp_if_constant_nonzeronegative_test2(float %x) {
; CHECK-LABEL: @fcmp_if_constant_nonzeronegative_test2(
...
define i1 @fcmp_if_constant_poszero_test4(float %x) {
; CHECK-LABEL: @fcmp_if_constant_poszero_test4(
; CHECK-NEXT: [[MUL:%.*]] = fmul float %x, 0.0
; CHECK-NEXT: [[CMP:%.*]] = fcmp ogt [[MUL]], 0.0
; CHECK-NEXT: ret i1 false
;
  %mul = fmul float %x, 0.0
  %cmp = fcmp ogt float %mul, 0.0
  ret i1 %cmp
}

define i1 @fcmp_operandposition_switch_test5(float %x) {
; CHECK-LABEL: @fcmp_operandposition_switch_test5(
; CHECK-NEXT: [[CMP:%.*]] = fcmp olt float [[X:%.*]], 0.0
;
  %mul = fmul float 5.0, %x
  %cmp = fcmp ogt float 0.0, %mul
  ret i1 cmp
}
